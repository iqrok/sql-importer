...
function regexForColDef () {
	return {
		...
		CHARACTER: '(?:\\s{1,}CHARACTER.*?SET\\s{1,}[\\w]+\\s{1,}[\\w]+\\s{1,}\\w+){0,}',
...
function colDefFromObject(obj) {
	...
	res.def += (obj.isNullable ? 'NULL' : 'NOT NULL');
	res.def += (obj.default ? `DEFAULT ${obj.default} ` : '');
	res.def += (obj.isAutoIncrement ? `AUTO_INCREMENT` : '');

	res.def = res.def.trim();
