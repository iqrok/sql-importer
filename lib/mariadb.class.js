/**
 * @file ./mariadb.class.js
 * */
const mariadb = require('mariadb');
const crypto = require('crypto');
/**
 * @defgroup MARIADB MariaDB Helper
 * @brief Helper to operate with MariaDB nodejs connector.
 * */

'use strict';

/**
 * @param msg {**string**} string to be hashed
...
/**
 * @ingroup MARIADB
 * @class MariadbHelper lib/mariadb.class.js
 * @brief Helper for MariaDB nodejs connector.
 * */
class MariadbHelper {
	/**
	 * @private
	 ...
	 * @return {**Object**} current connection
	 * */
	connection() {
	 ...
	 * @param args {**string[]**} arguments to be passed into query
	 ...
	 * @param args {**string[]**} escaped arguments to be passed into query (avoiding injection)
	 * @param stripMeta {**bool**} Strip meta from result. Default to **false**
	 * @param dateStrings {**bool**} if false datetime columns will be returned as js Date object.
	 * 								Default to **true**.
	 * @returns {**Object[]**} sql query result
	 * */
	query(sql, args =[], stripMeta = false, dateStrings = true) {
	 ...
	 * @returns {**Object[]**} sql query result
	 * */
	batch(sql, args, dateStrings = true) {
		return new Promise(async (resolve, reject) => {
			//create pool and add it to global to minimize number of same connection in mysql
			if (!__sqlPools[this._connectionHash]) {
				__sqlPools[this._connectionHash] = await mariadb.createPool(this._config);
	 ...
	 * @param sql {**string**} sql query
	 * @param args {**string[]**} escaped arguments to be passed into query (avoiding injection)
	 * @return {**Object[]**} sql query with arguments
	 * */
	debug(sql, args) {
