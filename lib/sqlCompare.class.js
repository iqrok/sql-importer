...
class SQLCompare {
	...
	_tableInfo(parsed) {
		...
		for (const name in parsed.table) {
			...
			for (const colName in info) {
				const detail = info[colName];
				tables[name].columns[colName] = parseColumnCreateQuery(detail);
			}
		}
	 ...
	 *
	 * @private
	 * @param source {#ColumnDef{}} reference source column
	 * @param target {#ColumnDef{}} target column to be compared
	 *
	 * @returns {**Boolean**} **true** If both columns are identical. Otherwise **false**
	 * */
	_compareColumn(source, target) {
		const self = this;
		...
		for (const key in source) {
			...
			if (type !== typeof(target[key])) {
				...
				case 'array':
				case 'object': {
					isSame = objDeepCompare(source[key], target[key]);
					if (isSame) {
						delete source[key];
						delete target[key];
					}
				...
				default: {
					isSame = source[key] === target[key];
				} break;

				}
			}

			const color = isSame ? '\x1b[32m' : '\x1b[1m\x1b[91m';

			if(self._config.verbose > 0) {
				console.log(`  ${color}> Comparing COLUMN ${key} [${isSame}]\x1b[0m`);
			}
	...
	compare(source, target) {
		...
		for (const tbl in source) {
			if(self._config.verbose > 0) {
				console.log('\x1b[1m> Comparing TABLE', tbl, '\x1b[0m');
			}

			const isSame = self._compareColumn(
					source[tbl].columns,
					target[tbl].columns
				);
	...
	printDiff(res) {
		...
		for (const tbl in diff) {
			...
			console.info(`> Table \x1b[1m${tbl}\x1b[0m\n`);
			...
			for (const col in diff[tbl]) {
				...
				console.info(`  source ${prefix.src} ${source ?? ''.padStart(target.length, ' ')}\x1b[0m`);
				console.info(`  target ${prefix.tar} ${target ?? ''.padStart(source.length, ' ')}\x1b[0m\n`);
			}
		}
	}
