...
class SQLCompare {
	 ...
	 *
	 * @details source and target will be modified at the end of the function.
	 *         Both objects will contains unmatched columns only.
	 *
	 * @private
	 ...
	 *
	 * @details source and target will be modified at the end of the function.
	 *         Both objects will contains unmatched tables only.
	 *
	 * @public
	 * @param[in,out] source {#ColumnDef{}} reference source column
	 * @param[in,out] target {#ColumnDef{}} target column to be compared
	...
	/**
	 * get difference report between both databases as a string.
	 *
	 * @details The result is only trying to mimic diff format,
	 * but not an actual valid diff format.
	 *
	 * Symbol usage in the report is as follows:
	 * |Symbol|Note|
	 * |------|----|
	 * |\-\-|a column is removed from source|
	 * |++|a column is added in target|
	 * |<<|a column is modified, this notes original content in source|
	 * |>>|a column is modified, this notes modified content in target|
	 *
	 * @public
	 * @param res {**Object**} #compare result.
	 *
	 * @return {**string**} diff report as string
	 * */
	getDiffString(res, colorize = true) {
		if (res.status) return 'Identical Database';
		...
		const color = {
			green(str) { return colorize ? `\x1b[1m\x1b[46m${str}\x1b[0m` : str },
			red(str) { return colorize ? `\x1b[1m\x1b[41m${str}\x1b[0m` : str },
			yellow(str) { return colorize ? `\x1b[1m\x1b[33m${str}\x1b[0m` : str },
			blue(str) { return colorize ? `\x1b[1m\x1b[34m${str}\x1b[0m` : str },
			bright(str) { return colorize ? `\x1b[1m${str}\x1b[0m` : str },
		};

		for (const tbl in diff) {
			str += `@@Table ${color.bright(tbl)}\n`;
			...
			for (const col in diff[tbl]) {
				...
				const addColor = {
					src: prefix.src === '--' ? color.red : color.yellow,
					tar: prefix.tar === '++' ? color.green : color.blue,
				};

				const text = {
					src: addColor.src(`${prefix.src} source: ${source ?? ''}`),
					tar: addColor.tar(`${prefix.tar} target: ${target ?? ''}`),
				};
