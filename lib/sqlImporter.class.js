...
class IMPORTER {
	...
	async _compareTables(queries) {
		...
		for(const tbl of queries.sort){
			...
			const _same = [];
			const _mod = [];
			const _nomore = Object.keys(existing[tbl]);

			for(const reqCol in requested){
				const now = self._parseColumnCreateQuery(requested[reqCol]);
				const old = existing[tbl][reqCol];

				if (old !== undefined) {
					// remove from no_more
					const idx = _nomore.indexOf(reqCol);
					_nomore[idx] = undefined;

					if (self._isColumnChanged({ now, old })) {
						_same.push({ name: reqCol, detail: requested[reqCol] });
					} else {
						_mod.push({ name: reqCol, detail: requested[reqCol] });
						console.log('---->', reqCol, { now, old }, requested[reqCol]);
					}
				} else {
					_new.push({ name: reqCol, detail: requested[reqCol] });
				}
			}
