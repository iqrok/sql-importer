...
class IMPORTER {
	...
	_parseColumnCreateQuery(query) {
		const regex = /^([0-9a-z_]+)(?:\({0,}([\d]+){0,}\){0,}\s{0,})(?:(NOT NULL|NULL){0,}\s{0,})(?:DEFAULT\s+([^\s]+)\s{0,}){0,}/mi;
		...
		return {
			...
			datatype: match[1],
			length: match[2] ? Number(match[2]) : 0 ,
			is_nullable: match[3] === 'NULL' || match[4] === 'NULL',
			default: match[4] || undefined,
		};
	}

	_compareColumnsIdentity({ target, source }) {
	...
	async importFile(options = {}){
		...
		let { withData, dropFirst, closeConnection, compareExisting } = options;
		...
		if(compareExisting){
			...
			`,[self._config.database])
				.then(res => {
					...
					for(let i = 0; i < tblLen; i++){
						...
						const isInTable = curTbl in tables;
						if(!isInTable) tables[curTbl] = {};
						...
						const newObj = {
							[curCol]: {
								...
								length: Number(res[i].character_maximum_length),
								is_nullable: res[i].is_nullable.toUpperCase() === 'YES',
								default: res[i].column_default,
							},
						};
			...
			for(let tbl of queries.sort){
				if(!(getExistTables[tbl] && queries.table[tbl])) continue;

				const columns = self._getColumnsFromCreateTable(queries.table[tbl][0]);
				const existingCols = Object.keys(getExistTables[tbl]);
				const nonExistents = [];
				sameTables[tbl] = [];
				...
				for(const column in columns){
					if(column in getExistTables[tbl]){
						// remove existing column from list
						const idx = existingCols.indexOf(column);
						existingCols[idx] = undefined;
					} else {
						nonExistents.push(column);
					}
				}

				for (const dst of nonExistents) {
					if(dst === undefined) continue;

					const info = {
						source: self._parseColumnCreateQuery(columns[dst]),
						target: undefined,
					};

					const matches = [];
					for(const src of existingCols) {
						if(src === undefined) continue;

						info.target = getExistTables[tbl][src];

						if (self._compareColumnsIdentity(info)) {
							matches.push(src);
						}
					}
					...
					case 1: {
						const { target } = info;
						queries.alter.push(`ALTER TABLE ${tbl}`
							+ ` CHANGE ${matches.pop()} ${dst}`
							+ ` ${target.type.toUpperCase()}`
							+ ` ${target.is_nullable ? 'NULL' : 'NOT NULL'}`
							+ ` DEFAULT ${target.default}`);
					} break;

					// matches more than 1 column. Do nothing for now
					default: {
