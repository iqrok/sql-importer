...
class IMPORTER {
	...
	_insertMultipleToSingle(query){
		...
		for(const val of values){
			inserts.push('INSERT INTO `' + table + '` ' + columns
				+ ' VALUES ' + val + ';');
		}
	...
	async parse(filepath){
		...
		for(let idx = 0; idx < length; idx++){
			...
			if(_queries[idx].match(/^INSERT\s/i)){
				const table = self._getNameFromQuery('INSERT', _queries[idx]);
				if(queries.insert[table] === undefined){
					queries.insert[table] = [];
				}

				queries.insert[table].push(_queries[idx]);
	...
	topologySortDependencies(queries){
		...
		do {
			...
			i = 0;
			while (i < keys.length) {
				if (tDep[keys[i]].every(Set.prototype.has, used)) {
	...
	async importFile(filepath, options = {}){
		...
		const tDep = self.topologySortDependencies(queries.alter);
