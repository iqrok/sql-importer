...
class IMPORTER {
	...
	_alterMultipleToSingle(query){
		...
		const regex = /(?![^a-z])((?:ADD|MODIFY)\s{0,}.*?\(.*?\)(?:\s{1,}(?:USING.*?(?:[\w|\s]+)|REFERENCES.*?\(.*?\)(?:\s{1,}ON.*?(?:[\w|\s]+)))){0,})/gmi;
	...
	async _processSameTables(queries, sameTables) {
		const self = this;
	...
	async _compareTables(queries) {
		...
		const existingTables = await self._mysql.query(`
	...
	async importFile(options = {}){
		...
		if(compareExisting){
			const sameTables = await self._compareTables(queries);
			if(sameTables) self._processSameTables(queries, sameTables);
		}
